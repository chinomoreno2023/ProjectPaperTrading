...
class DirectWritingPortfolioIfItIsContainedTest {
    ...
    @InjectMocks
    private DirectWritingPortfolioIfItIsContained strategy;

    @Test
    void getType_ShouldReturnStrategyType() {
        assertEquals(AdditionStrategyType.DIRECT_WRITING_IF_CONTAINED, strategy.getType());
    }
    ...
    void whenEnoughMoney_thenUpdatesPortfolioAndSendsEvent() {
        ...
        when(positionSetter.isThereEnoughMoneyForWrite(newPortfolio)).thenReturn(true);
        ...
        assertAll(
                ...
                () -> assertNotNull(AnnotationUtils.findAnnotation(
                        DirectWritingPortfolioIfItIsContained.class.getMethod("addPortfolio", OptionDto.class),
                        Transactional.class))
        );
    }
    ...
    void whenNotEnoughMoney_thenThrowsInsufficientFundsException() {
        ...
        when(personsService.getCurrentPerson()).thenReturn(person);
        when(newPortfolio.getOption()).thenReturn(option);
        lenient().when(portfoliosRepository.findByOwnerAndOption(person, option)).thenReturn(oldPortfolio);
        when(positionSetter.isThereEnoughMoneyForWrite(newPortfolio)).thenReturn(false);
